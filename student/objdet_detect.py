...
def load_configs_model(model_name='darknet', configs=None):
    ...
    if model_name == "darknet":
        ...
        configs.num_layers = 18 # custom flag
        ...
        # cant see these being used anywhere in code
        # do we need to copy them?
        configs.use_giou_loss = False

        # seem related to SFA3D only
        configs.batch_size = 4
...
def load_configs(model_name='fpn_resnet', configs=None):
    ...
    # fixes: 'EasyDict' object has no attribute 'min_iou' File "/loop_over_dataset.py", line 183
    configs.min_iou = 0.5
...
def detect_objects(input_bev_maps, model, configs):
    ...
    y_px_to_m = bound_size_y / configs.bev_width
    x_px_to_m = bound_size_x / configs.bev_height
    ...
    if len(detections) > 0:
        ...
        for detection in detections:
            ...
            _, _x, _y, z, _h, _w, _l, _yaw = detection

            _yaw = -_yaw
            x = _y * x_px_to_m
            y = _x * y_px_to_m - bound_size_x/2.0
            w = _w * y_px_to_m
            l = _l * x_px_to_m

            ## step 4 : append the current object to the 'objects' array
            objects.append([class_to_show, x, y, z, _h, w, l, _yaw])
